...
import {createPosTargetShader, createAccShader, createVelShader, createPosShader} from './entangled-shaders.js';
...
function init() {
    console.log("Initializing Entangled...");
    initialized = true;
...
function render() {
    ...
    // Stochastic sync - probabilistic coupling when paired
    if (isPaired) {
        if (Math.random() < 0.1) { // 10% chance per frame
            syncIntensity += Math.random() * 0.15;
        }
        syncIntensity *= 0.95; // Decay
    } else {
        syncIntensity *= 0.9;
    }
    ...
    // Rotation like Bjørn's original
